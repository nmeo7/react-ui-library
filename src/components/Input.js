...
export const Input = (props) => {
    ...
    return (
        <div style={{ width: props.width || 'calc(100% - 4px)', paddingBottom: '1em'  }} >
            ...
            {
                props.textarea ?
                <textarea { ...style } {...input_style} style={{ height: '200px', resize: 'vertical' }} placeholder={props.label} onChange={ e => props.onChange(e.target.value) } ></textarea>
                :
                <input { ...style } {...input_style} placeholder={ `type ${props.label} here` || 'Label...'} type={ props.type || 'text' }  onChange={ e => props.onChange(e.target.value) } />
            }
        </div>
    )
}